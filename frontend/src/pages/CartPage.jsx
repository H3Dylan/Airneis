...
const CartPage = () => {
    const { cart, addToCart, removeFromCart, deleteFromCart, updateQuantity, clearCart } = useContext(CartContext);
    ...
    return (
        <>
            <Header />
            <div className="my-12">
                <h2 className="text-2xl text-center font-bold mb-4">Mon Panier</h2>
                <div className="flex flex-col md:flex-row md:mx-auto md:justify-evenly md:min-w-[600px]  md:max-w-5xl">
                    <div>
                        {cart.length > 0 ? (
                            cart.map((item, index) => (
                                <div key={index} className="flex justify-between items-center gap-1 text-center p-2 border-b border-gray-300">
                                    ...
                                    <div className="flex flex-col items-center gap-1">
                                        ...
                                        <div>
                                            <HiOutlineTrash size={30} onClick={() => deleteFromCart(item._id)} className='hover:cursor-pointer'/>
                                        </div>
                                    </div>
                                </div>
                            ))
                        ...
                        {cart.length > 0 && (
                            <div className="flex flex-col gap-2">
                                ...
                                <button className="btn_primary self-center p-2 rounded-lg">Passer la commande</button>
                            </div>
                        )}
                </div>
            </div>
